...
namespace MscThesis.Runner.Factories
{
    public abstract class TestFactory<T> : ITestFactory where T : Instance
    {
        ...
        public ITest BuildTest(TestSpecification spec)
        {
            ...
            foreach (var generator in generators)
            {
                ...
                foreach (var variableValue in variableValues)
                {
                    ...
                    else
                    {
                        ...
                        run = new RunsComposite(() => generator(problemSize.Value, saveSeries, varSpec), spec.NumRuns, runBatchSize, saveSeries, varSpec);
                    }
                ...
                {
                    sizeResult = new VariablesComposite(variableResults, sizeBatchSize, spec.Variable.Value);
                }
            ...
            {
                return new OptimizersComposite(tests, testBatchSize);
            }
        }
        ...
        private IEnumerable<Func<int, bool, VariableSpecification, ITest>> BuildRunGenerators(TestSpecification spec)
        {
            ...
            foreach (var optimizerSpec in spec.Optimizers)
            {
                ...
                var buildTerminationsFunc = new Func<int, VariableSpecification, IEnumerable<ITerminationCriterion>>((problemSize, varSpec) =>
                {
                    var terminations = new List<ITerminationCriterion>();
                ...
                yield return (int problemSize, bool saveSeries, VariableSpecification variableSpec) =>
                {
                    ...
                    var run = optimizer.BuildRun(problem);
